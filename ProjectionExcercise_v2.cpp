	...
	for(int i=0; i < sz; i++) {
	  ...
	  if(closestPoints[i].size() > 1) {
	    ...
	    for(int j=0; j < closestPoints[i].size(); j++) {
	      if (points[closestPoints[i][j]].x < minX) minX = points[closestPoints[i][j]].x;  
	      if (points[closestPoints[i][j]].y < minY) minY = points[closestPoints[i][j]].y;
	      if (points[closestPoints[i][j]].x > maxX) maxX = points[closestPoints[i][j]].x;  
	      if (points[closestPoints[i][j]].y > maxY) maxY = points[closestPoints[i][j]].y;
	    }
	    projection_objects.push_back(cv::Rect(minX,minY,maxX-minX,maxY-minY));
	  }
	}
      }
